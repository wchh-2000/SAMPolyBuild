...
class RSFeatureAggregator(BaseModule):
    ...
    def forward(self, inputs):
        ...
        _, inner_states = inputs
        features = [layer(x) for layer, x in zip(self.downconvs, inner_states)]
...
class SAMAggregatorNeck(nn.Module):
    def __init__(
        ...
        self.out_channels = out_channels
        self.stride = stride
        self.selected_channels = selected_channels#只在init中用到
    ...
    def forward(self, inputs):
        _, inner_states = inputs
        inner_states = [layer(x) for layer, x in zip(self.down_sample_layers, inner_states)]
        #list [b,768,64,64]->[b,32,32,32] inner_channels=32 为什么768降维到32？todo 更大的inner_channels
        x = None
        for inner_state, layer in zip(inner_states, self.fusion_layers):
            if x is not None:
                inner_state = x + inner_state
        ...
        return img_feats_2, img_feats_1, img_feats_0 #[b,256,128,128],[b,256,64,64],[b,256,32,32]