...
class BasePLer(pl.LightningModule, BaseModel):
            ...
            if hasattr(self, f'{evaluator}_evaluator'):
                if hasattr(self.trainer.datamodule, f'{evaluator}_dataset'):
                    ...
                    if hasattr(dataset, 'metainfo'):
                        ...
                        evaluator_.dataset_meta = dataset.metainfo
    ...
    def on_test_epoch_end(self) -> None:
        evaluator = getattr(self, f'test_evaluator')
        # n_batch=len(self.trainer.datamodule.test_dataloader())
        # n_imgs=len(self.trainer.datamodule.test_dataloader().dataset)
        # metric_file=evaluator.metric_file
        # avg_pos=round(self.panoptic_head.roi_head.pos_process_time/n_batch,2)        
        m = evaluator.compute()
        print(m)
        # with open(metric_file, 'a+') as f:
        #     f.write(f"total time: {round(self.total_time,2)}, FPS: {round(n_imgs/self.total_time,2)},")
    ...
    def _log_eval_metrics(self, stage):
        ...
        if hasattr(self, f'{stage}_evaluator'):
            evaluator = getattr(self, f'{stage}_evaluator')#CocoPLMetric
            metrics = evaluator.evaluate(size=len(self.trainer.datamodule.val_dataloader().dataset))#only for stage==val
            metrics = {k.lower(): v for k, v in metrics.items()}
            keys = []
            for k, v in metrics.items():
                # v = v.view(-1)
                if k == 'valmask_iou':
                    self.log(k, v, on_step=False, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
                    keys.append(k)
                k=k.replace('valmask_','valmask/')
                k=k.replace('valbbox_','valbbox/')
                k=k.replace('valpoly_','valpoly/')
                keys.append(k)
                self.log(k, v, on_step=False, on_epoch=True, prog_bar=False, logger=True, sync_dist=True)#_{i}
            # evaluator.reset()
            ...
            if hasattr(self.trainer, 'checkpoint_callback'):
                ...
                if (monitor is not None) and (monitor not in keys):
                    data = torch.tensor(0., device=self.device)#monitor字段没有时显示0
                    self.log(f'{monitor}', data, on_step=False, on_epoch=True, prog_bar=False, logger=True, sync_dist=True)