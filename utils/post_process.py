...
def GetPolygons(pred_segs, pred_vmaps, pred_voffs, ori_size=(512, 512),
    ...
    candidate_vertices = [get_candidate_vertex(pred_vmaps[b], pred_voffs[b]) for b in range(num_instances)]
    # candidate_vertices =get_candidate_vertex(pred_vmaps, pred_voffs)
    scale_size=(ori_size[1]/pred_vmaps.shape[2], ori_size[0]/pred_vmaps.shape[3])#原图/特征图尺寸=4 crop输入时原图大小为224
    args = [
        (
    ...
    for result in results:        
        if result is not None:
            poly, best_score, idx = result #idx限定了位置，imap_unordered，不会改变顺序
            batch_polygons[idx] = poly
            batch_scores[idx] = best_score
            valid_mask[idx] = True  

    return batch_polygons, batch_scores, valid_mask
